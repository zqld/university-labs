...
public class CopyUtil3 {
    public static void copy(final InputStream src, final OutputStream dst) throws IOException {
        ...
        final BlockingQueue<byte[]> emptyBuffer = new ArrayBlockingQueue<>(64); // пустой буфер, которые будет использоваться еще раз
        ...
        Thread reader = new Thread(group, () -> {
            try (InputStream src0 = src) {              // 'src0' for auto-closing (try with resouerces)
                while (true) {
                    byte[] data = emptyBuffer.poll(); // извлекает и удаляет элемент из начала очереди, возвращая null если очередь пуста
                    if(data == null) data = new byte[128]; // проверяем, если нет свободного буфера, создает новый
        ...
        Thread writer = new Thread(group, () -> {
            try (OutputStream dst0 = dst) {      // 'dst0' for auto-closing (try with resouerces)
                while (true) {
                    ...
                    emptyBuffer.offer(data); // добавляем в очередь без блокировок (если очередь не заполнена), возвращает true или false
                }

...
public class Test {
    public static void main(String[] args) throws IOException {
        ...
        // для проверки
        byte[] a = dst.toByteArray();
        byte[] b = dst1.toByteArray();
        byte[] c = dst2.toByteArray();
        byte[] d = dst3.toByteArray();

        if (!Arrays.equals(testData, a) || !Arrays.equals(testData, b) || !Arrays.equals(testData, c) || !Arrays.equals(testData, d)) { // проверка совпадления поток (toByteArray возвращает содержимое потока в виде массива байтов)

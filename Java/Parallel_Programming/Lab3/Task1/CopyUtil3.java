...
Перепишите код, что бы чтение осуществлялось в новом потоке, 
а запись производилась потоком, вызвавшим copy(...). 
Кстати, тогда можно будет избавиться от пары join-ов, 
так как поток на принимающем конце буфера знает, когда закончились данные. */
